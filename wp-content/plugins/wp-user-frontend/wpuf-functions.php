...
function wpuf_show_post_status( $status ) {
    if ( 'publish' === $status ) {
        $title     = __( 'Опубликован', 'wp-user-frontend' );
        $fontcolor = '#33CC33';
    } elseif ( 'draft' === $status ) {
        $title     = __( 'Черновик', 'wp-user-frontend' );
        $fontcolor = '#bbbbbb';
    } elseif ( 'pending' === $status ) {
        $title     = __( 'Ожидает одобрения', 'wp-user-frontend' );
        $fontcolor = '#C00202';
    } elseif ( 'future' === $status ) {
        $title     = __( 'Запланирован', 'wp-user-frontend' );
        $fontcolor = '#bbbbbb';
    } elseif ( 'private' === $status ) {
        $title     = __( 'Приватный', 'wp-user-frontend' );
        $fontcolor = '#bbbbbb';
    }
