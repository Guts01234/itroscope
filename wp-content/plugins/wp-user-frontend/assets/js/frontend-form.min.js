(function ($, window) {
    ...
    window.WP_User_Frontend = {
        init: function () {
            ...
            if (window.matchMedia('(max-width: 600px)').matches) {
                ...
                $('.wpuf-dashboard-navigation ul li:first-child')
                    .after()
                    .click(function (e) {
                        e.preventDefault();
        ...
        warningOnExit: function () {
            ...
            $('.wpuf-form-add').on('change', function () {
                $('.wpuf-form-add input, .wpuf-form-add select, .wpuf-form-add textarea').each(
                    function (index) {
                        if (
        ...
        enableMultistep: function (o) {
            ...
            if (
                progressbar_type == 'progressive' &&
                $('.wpuf-form .wpuf-multistep-fieldset').length != 0
            ) {
                var firstLegend = $('fieldset.wpuf-multistep-fieldset legend').first();
                $('.wpuf-multistep-progressbar').html(
                    '<div class="wpuf-progress-percentage"></div>',
                );
            ...
            $('fieldset .wpuf-multistep-prev-btn, fieldset .wpuf-multistep-next-btn').click(
                function (e) {
                    // js_obj.formSubmit();
        ...
        change_fieldset: function (step_number, progressbar_type) {
            ...
            $('.wpuf-step-wizard li').each(function () {
                if ($(this).index() <= step_number) {
                    progressbar_type == 'step_by_step'
                        ? $(this).addClass('passed-wpuf-ms-bar')
                        : $('.wpuf-ps-bar', this).addClass('passed-wpuf-ms-bar');
                } else {
                    progressbar_type == 'step_by_step'
                        ? $(this).removeClass('passed-wpuf-ms-bar')
                        : $('.wpuf-ps-bar', this).removeClass('passed-wpuf-ms-bar');
                }
            });
            ...
            $('.passed-wpuf-ms-bar')
                ...
                .last()
                .addClass('wpuf-ms-bar-active');
            $('.wpuf-ms-bar-active').addClass('active-step');

            var legend = $('fieldset.wpuf-multistep-fieldset')
                .eq(step_number)
                .find('legend')
                .text();
            legend = $.trim(legend);
            ...
            if (
                ...
                var progress_percent =
                    ((step_number + 1) * 100) / $('fieldset.wpuf-multistep-fieldset').length;
                ...
                success: function (html) {
                    ...
                    if (html != '') {
                        $(dropdown)
                            ...
                            .addClass('hasChild')
                            .parent()
                            .append('<div id="' + container_id + '" level="' + level + '"></div>');
                        dropdown
                            .parent()
                            .parent()
                            .find('#' + container_id)
                    ...
                    $(document).trigger(
                        'wpuf-ajax-fetched-child-categories',
                        container_id,
                        level,
                        taxonomy,
                    );
        ...
        draftPost: function (e) {
            ...
            $.post(wpuf_frontend.ajaxurl, form_data, function (res) {
                if (typeof post_id === 'undefined') {
                    ...
                    html +=
                        '<input type="hidden" name="post_author" value="' + res.post_author + '">';
                    html +=
        ...
        formSubmit: async function (e) {
            ...
            //получаем форму в которой будет происходит валидация
            const form_type = $('input[name="type_form"]:checked').val();

            function checkNotEmptyInput($el) {
                const value = $el.val();
            ...
            function modalErrorShow(err) {
                $('#modal_error_add_post').fadeIn();
                $('#modal_error_add_post .content').html(err);
                setTimeout(() => {
            ...
            switch (form_type) {
                //валидация урока
                case 'lesson':
                    const lesson_cart_state = $('input[name="lesson_cart_state"]:checked').val();

                    if (lesson_cart_state == 'take') {
                        let val_title = $('#create_lesson_form_take input[name="post_title"]').val()
                            .length;
                        let val_navik = $(
                            '#create_lesson_form_take .navik_teory_take input[name="navik"]',
                        ).val().length;
                        let val_problem = $(
                            '#create_lesson_form_take .problem_teory_take input[name="problem"]',
                        ).val().length;
                        let price = $(
                            '#create_lesson_form_take input[name="post_price_take"]',
                        ).val();

                        if (val_title > 200 || val_navik > 25 || val_problem > 25) {
                            modalErrorShow('Вы превысили лимит');
                            return 0;
                        }
                        if (price != parseInt(price)) {
                            modalErrorShow('Неверно указана цена');
                            return 0;
                        }
                    } else if (lesson_cart_state == 'give') {
                        var items_p = $(`#post_content_${post_form_id}_ifr`)
                            .contents()
                            .find('body p');
                        var items_li = $(`#post_content_${post_form_id}_ifr`)
                            .contents()
                            .find('body li');
                        var count_elems = 0;
                        let val_title = $(`.wpuf_post_title_${post_form_id}`).val().length;
                        let val_post_homework = $(`.wpuf_post_homework_${post_form_id}`).val()
                            .length;
                        let val_navik = $('#navik').val().length;
                        let val_problem = $('#problem').val().length;
                        let val_navik_practice = $(`.wpuf_navik_practice_${post_form_id}`).val()
                            .length;
                        let val_problem_practice = $(`.wpuf_problem_practice_${post_form_id}`).val()
                            .length;
                        let youtube_href = $(`#youtube_href_${post_form_id}`).val();
                        ...
                        let has_teory = $('input[name="has_this_post_teory"]:checked').val();
                        let has_pracice = $('input[name="has_this_post_homework"]:checked').val();
                        if (has_teory == undefined) {
                            $('#modal_error_add_post').fadeIn();
                            $('#modal_error_add_post .content').html(
                                'Пожалуйста, выберете, будет ли теория в этом уроке.',
                            );
                            setTimeout(() => {
                                $('#modal_error_add_post').fadeOut();
                            }, 2000);
                        ...
                        if (has_pracice == undefined) {
                            $('#modal_error_add_post').fadeIn();
                            $('#modal_error_add_post .content').html(
                                'Пожалуйста, выберете, будет ли практика в этом уроке.',
                            );
                            setTimeout(() => {
                                $('#modal_error_add_post').fadeOut();
                            }, 2000);
                        ...
                        if (has_teory == 0 && has_pracice == 0) {
                            $('#modal_error_add_post').fadeIn();
                            $('#modal_error_add_post .content').html(
                                'Нельзя создать урок без теории и практики!',
                            );
                            setTimeout(() => {
                                $('#modal_error_add_post').fadeOut();
                            }, 2000);
                        ...
                        if (
                            count_elems > 5000 ||
                            val_title > 200 ||
                            val_post_homework > 200 ||
                            val_navik > 25 ||
                            val_problem > 25 ||
                            val_navik_practice > 25 ||
                            val_problem_practice > 25
                        ) {
                            $('#modal_error_add_post').fadeIn();
                            $('#modal_error_add_post .content').html(
                                'Вы превысили лимит, пожалуйста, исправьте!',
                            );
                            setTimeout(() => {
                                $('#modal_error_add_post').fadeOut();
                            }, 2000);
                        ...
                        if (youtube_href) {
                            let video_duration;
                            try {
                                video_duration = await $.ajax({
                                    url: site_url_header + '/wp-admin/admin-ajax.php',
                                    data: {
                                        action: 'ytD',
                                        video_url: youtube_href,
                                    },
                                    type: 'POST',
                                });
                            } catch (e) {
                                ...
                                $(`#youtube_status_vidio_${post_form_id}`).val('');
                                setTimeout(() => {
                                    $('#modal_error_add_post').fadeOut();
                                }, 2000);
                                return;
                            ...
                            if (video_min > 15) {
                                $(`#youtube_status_vidio_${post_form_id}`).val('long');
                                const res = confirm(
                                    'Вы добавляете длинное видео. Мы рекомендуем не делать видео-уроки длиннее 15 минут. Вы можете оставить длинное видео на ваше усмотрение. Продолжить сохранение?',
                                );
                                if (!res) {
                                    return;
                                }
                            } else {
                                $(`#youtube_status_vidio_${post_form_id}`).val('short');
                            }
                        }
                    }
                ...
                case 'gift':
                    //заголовок
                    const titleLenght = $('.gift_desc input[name="post_title"]').val().length;

                    // отдать или получить
                    const cart_state = $(
                        '#create_gift_form input[name="cart_state"]:checked',
                    ).val();

                    if (titleLenght <= 0) {
                        modalErrorShow('Вы не указали описание объявления');
                        return;
                    }
                    ...
                    const gift_type = $('input[name="type_gift"]:checked').val();

                    if (!checkNotEmptyInput($('input[name="type_gift"]:checked'))) {
                        modalErrorShow('Вы не выбрали тип подарка');
                        return;
                    }

                    switch (gift_type) {
                        case 'money':
                            if (!checkNotEmptyInput($('input[name="gift_money_sum"]'))) {
                                modalErrorShow('Вы не указали сумму денег');
                                return;
                            }

                            if (
                                cart_state == 'take' &&
                                !checkNotEmptyInput($('input[name="gift_money_transaction"]'))
                            ) {
                                modalErrorShow('Вы не указали данные для получения денег');
                                return;
                            }
                            break;

                        case 'product':
                            const gift_product_type = $(
                                'input[name="gift_product_type"]:checked',
                            ).val();
                            if (!checkNotEmptyInput($('input[name="gift_product_type"]:checked'))) {
                                modalErrorShow('Вы не указали тип подарка');
                                return;
                            }

                            if (!checkNotEmptyInput($('input[name="gift_product_price"]'))) {
                                modalErrorShow('Вы не указали цену');
                                return;
                            }

                            if (gift_product_type == 'internet') {
                                if (!checkNotEmptyInput($('input[name="ozon_link"]'))) {
                                    modalErrorShow('Вы не указали ссылку на товар');
                                    return;
                                }
                            }
                ...
                case 'time':
                    //заголовок
                    const titleLenghtTime = $('#create_time_form input[name="post_title"]').val()
                        .length;
                    let val_navik_time = $('.navik_time input').val().length;
                    let val_problem_time = $('.problem_time input').val().length;

                    if (titleLenghtTime <= 0) {
                        modalErrorShow('Вы не указали описание объявления');
                        return;
                    }

                    if (titleLenghtTime > 200) {
                        modalErrorShow('Вы превысили лимит');
                        return 0;
                    }

                    if (
                        !checkNotEmptyInput($('#create_time_form input[name="cart_state"]:checked'))
                    ) {
                        modalErrorShow('Вы не выбрали хотите ли получить или отдать');
                        return;
                    }

                    if (!checkNotEmptyInput($('input[name="time_how_much"]:checked'))) {
                        modalErrorShow('Вы не выбрали сколько времени');
                        return;
                    }

                    var checkboxesMeeting = [];
                    $('input[name="time_meeting[]"]:checked').each(function () {
                        checkboxesMeeting.push($(this).val());
                    });

                    if (checkboxesMeeting.length <= 0) {
                        modalErrorShow('Вы не выбрали как Вы готовы к контакту');
                        return;
                    }

                    if (checkboxesMeeting.includes('offline')) {
                        if (!checkNotEmptyInput($('input[name="time_town"]'))) {
                            modalErrorShow('Вы не указали город');
                            return;
                        }
                    }
                    if (val_navik_time > 25 || val_problem_time > 25) {
                        $('#modal_error_add_post').fadeIn();
                        $('#modal_error_add_post .content').html(
                            'Вы превысили лимит, пожалуйста, исправьте!',
                        );
                    ...
                    console.log('awdwdq 13');
                    break;

                default:
                    console.log('awdwdq 12');
        ...
        validateForm: function (self) {
            ...
            var visibleFields = self.find(
                'input:visible, textarea:visible, select:visible, .wpuf-rich-validation',
            );
                    ...
                    case 'text':
                        ...
                        if (WP_User_Frontend.isTelephoneField(item)) {
                            ...
                            $('<input>')
                                ...
                                .attr('name', name)
                                .attr('value', value)
                                .appendTo(self); // append the mobile number with country code
                        ...
                        if (
                            input_type === 'url' &&
                            val !== '' &&
                            !WP_User_Frontend.isValidURL(val)
                        ) {
                            error = true;
                            error_type = 'validation';
                            WP_User_Frontend.markError(item, error_type);
        ...
        addErrorNotice: function (form, position) {
            if (position == 'bottom') {
                $('.wpuf-multistep-fieldset:visible').append(
                    '<div class="wpuf-errors">' + wpuf_frontend.error_message + '</div>',
                );
            } else {
                $(form)
                    .find('li.wpuf-submit')
                    .append('<div class="wpuf-errors">' + wpuf_frontend.error_message + '</div>');
            }
        },
        ...
        markError: function (item, error_type) {
            ...
            if (error_type) {
                ...
                if (
                    $(item).prev().prop('nodeName') === 'DIV' &&
                    !$(item).hasClass('wpuf_telephone')
                ) {
        ...
        isValidURL: function (url) {
            var urlregex = new RegExp(
                '^(http:\\/\\/www\\.|https:\\/\\/www\\.|http:\\/\\/|https:\\/\\/){1}[a-z0-9]+([\\-\\.]{1}[a-z0-9]+)*\\.[a-z]{2,20}(:[0-9]{1,20})?(\\/.*)?$',
            );
        ...
        deleteAvatar: function (e) {
            ...
            if (confirm($(this).data('confirm'))) {
                $.post(
                    ...
                    { action: 'wpuf_delete_avatar', _wpnonce: wpuf_frontend.nonce },
                    function () {
        ...
        editorLimit: {
            bind: function (limit, field, type, limit_type, limit_to) {
                if (type === 'no') {
                    ...
                    $('textarea#' + field).keydown(function (event) {
                        WP_User_Frontend.editorLimit.textLimit.call(
                            this,
                            event,
                            limit,
                            limit_type,
                    ...
                    setTimeout(function () {
                        tinyMCE.get(field).onKeyDown.add(function (ed, event) {
                            WP_User_Frontend.editorLimit.tinymce.onKeyDown(
                                ed,
                                event,
                                limit,
                                limit_type,
                        ...
                        tinyMCE.get(field).onPaste.add(function (ed, event) {
                            setTimeout(function () {
                                WP_User_Frontend.editorLimit.tinymce.onPaste(
                                    ed,
                                    event,
                                    limit,
                                    limit_type,
            ...
            checkTinyMCERestriction: function (field, data, ed) {
                ...
                var limit =
                    typeof data[0] !== 'undefined' && data[0] !== null ? parseInt(data[0]) : 0;
                var fieldId =
                    typeof data[1] !== 'undefined' && data[1] !== null ? data[1].trim() : '';
                var fieldType =
                    typeof data[2] !== 'undefined' && data[2] !== null ? data[2].trim() : '';
                var limit_type =
                    typeof data[3] !== 'undefined' && data[3] !== null ? data[3].trim() : '';
                var limit_to =
                    typeof data[4] !== 'undefined' && data[4] !== null ? data[4].trim() : '';
                ...
                if (numWords > 1 && numWords > limit && 'max' === limit_to) {
                    ...
                    jQuery('.mce-path-item.mce-last', ed.container).html(
                        wpuf_frontend['word_' + limit_to] + ' ' + numWords + '/' + limit,
                    );
            ...
            tinymce: {
                ...
                onKeyDown: function (ed, event, limit, limit_type, limit_to) {
                    ...
                    var numWords = WP_User_Frontend.editorLimit.tinymce.getStats(ed).chars + 1,
                        limit_label =
                            'word' === limit_type ? 'Word Limit : ' : 'Character Limit : ';
            ...
            blockTyping: function (event) {
                ...
                if (
                    $.inArray(event.keyCode, [46, 8, 9, 27, 13, 110, 190, 189]) !== -1 ||
                    // Allow: Ctrl+A
    ...
    $(function () {
        $('input[name="first_name"], input[name="last_name"]').on('change keyup', function () {
            var myVal,
                newVal = $.makeArray(
                    $('input[name="first_name"], input[name="last_name"]').map(function () {
                        if ((myVal = $(this).val())) {
                            return myVal;
                        }
                    }),
    ...
    $(function ($) {
        ...
        $('.wpuf-form-add input[name="shopurl"]').keydown(function (e) {
            ...
            if (
                $.inArray(e.keyCode, [46, 8, 9, 27, 13, 91, 109, 110, 173, 189, 190]) !== -1 ||
                // Allow: Ctrl+A
            ...
            if (
                (e.shiftKey ||
                    ((e.keyCode < 65 || e.keyCode > 90) && (e.keyCode < 48 || e.keyCode > 57))) &&
            ...
            $.post(dokan.ajaxurl, data, function (resp) {
                if (resp == 0) {
                    ...
                    $('#url-alart-mgs')
                        ...
                        .text(dokan.seller.notAvailable);
                } else {
                    $('#url-alart').removeClass('text-danger').addClass('text-success');
                    $('#url-alart-mgs')
                        .removeClass('text-danger')
                        .addClass('text-success')
                        .text(dokan.seller.available);
                }
            });
        });
