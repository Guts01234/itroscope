...
if ( ! class_exists( 'um\core\User' ) ) {
	...
	class User {
		 ...
		 * user town
		 */
		function get_user_town(){
			$town = get_user_meta($this->id, 'user_towm_meta', true);
			if(!$town){
				$town = 'не указано';
			}
			return $town;
		 ...
		 * $town - string
		 */
		function update_user_town($town){
			return update_user_meta($this->id, 'user_towm_meta', $town);
		}


		function ajax_update_town(){
			$town = $_POST['town'];
			$u_id = $_POST['user_id'];
			update_user_meta($u_id, 'user_towm_meta', $town);
		}
