...
function um_profile_header( $args ) {
	...
	<div class="um-header<?php echo esc_attr( $classes ); ?>">
		...
		<div class="um-profile-meta">
			...
			<div class="um-profile-meta__left">
				<?php
				/**
				 * UM hook
				 *
				...
				<div class="um-town">
					...
					if( UM()->fields()->viewing == true ){
						?>
							<div class='um-town-row'>
								<img src="<?php echo get_site_url()?>/wp-content/uploads/2023/04/location.png" alt="map marker">
								<p>Сейчас здесь: <?php echo UM()->user()->get_user_town()?></p>
							</div>
						<?php 
						...
						?>
							<div>
								<p>Укажите, где вы находитесь:</p>
								<input type="text" 
									id='um-meta-towm' 
									value='<?php echo UM()->user()->get_user_town()?>'
									placeholder='Город..'> 
							</div>
						<?php
					}
				...
				<?php
					if(is_company(um_user( 'ID' ))){
						do_action('um_company_header', array(um_user( 'ID' )));
					}else{
				?>
				<div class="um-status">
					<?php 
						$status = get_user_status(um_user("ID"));
					?>
					<p>Статус:	<?php echo $status?></p>
				</div>
				<?php 
				$company_arr = get_user_meta(um_user("ID"),"company_arr", true);

				if($company_arr && count($company_arr) >=1){
					?>
						...
						<p>Команда: 
							<?php 
								...
								foreach($company_arr as $company_id){
									$str_echo.= get_user_link_name($company_id) .", ";
								}
								$str_echo = substr($str_echo,0,-2);
								echo $str_echo;
							?>
						</p>
					</div>
				...
				<div class="um-profile-meta__right">
					<div class="um-profile-meta__right__give">
						<p class="um-meta-title">Подарил</p>
						<?php 
							$donation = get_user_meta(um_user("ID"), 'donate_money', true);
							$teach_time = get_user_meta(um_user("ID"), 'teach_time', true);
							if(!$donation){
								$donation = 0;
							}
							if(!$teach_time){
							...
							echo "<p>$donation ₽ и $teach_time часов</p>";
							
						?>

					</div>
					<div class="um-profile-meta__right__take">
						<p class="um-meta-title">Получил</p>
						<?php 
							$take_money = get_user_meta(um_user("ID"), 'take_money', true);
							$learn_time = get_user_meta(um_user("ID"), 'learn_time', true);
							if(!$take_money){
								$take_money = 0;
							}
							if(!$learn_time){
							...
							echo "<p>$take_money ₽ и $learn_time часов</p>";
						?>
					</div>
				</div>
			<div class="um-clear"></div>
